...
Once the user has removed the columns they want, they can choose to train a model using RF or LR, before the accuracy percentage is displayed. This would initially be low (i.e. less than 50%) due to the large dataset, so the model needed to be trained a few times using a continually reducing dataset size by omitting some of the older Football seasons until a dataset consisting of 10 years' worth of matches from all leagues provided a reasonable accuracy percentage.
